...
trait MiniboxBridgeTreeTransformer extends TreeRewriters with ScalacCrossCompilingLayer {
  ...
  class BridgePhase(prev: Phase) extends StdPhase(prev) {
    ...
    def apply(unit: CompilationUnit): Unit = {
      afterMiniboxBridge(new BridgeTransformer(unit).transformUnit(unit))
    }
  }
    ...
    protected def rewrite(tree: Tree): Result = {
      tree match {
        case defdef: DefDef => // if hasStorage(defdef) =>
          val preOverrides = beforeMiniboxBridgeNext(defdef.symbol.allOverriddenSymbols)
...
//          println(defdef.symbol.owner + "   " + defdef.symbol + "   " + preOverrides + "   " + postOverrides + "   " + bridgeSyms + "   " + bridgeSymsFiltered)
//          if (!bridgeSyms.isEmpty) {
//            println()
//            println(defdef.symbol.defString)
//            for (sym <- preOverrides)
//              println(sym.defString + "  from  " + sym.owner + "   " + sym.allOverriddenSymbols)
//            println()
//            for (sym <- postOverrides)
//              println(sym.defString + "  from  " + sym.owner + "   " + sym.allOverriddenSymbols)
//            println()
//            for (sym <- bridgeSyms)
//              println(sym.defString + "  from  " + sym.owner + "   " + sym.allOverriddenSymbols)
//            println()
//            for (sym <- bridgeSymsFiltered)
//              println(sym.defString + "  from  " + sym.owner)
//            println()
//            println()
//          }
