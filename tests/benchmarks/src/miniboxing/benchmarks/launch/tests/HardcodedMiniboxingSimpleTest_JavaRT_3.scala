...
import miniboxing.benchmarks.hardcoded.java_rt_3._

trait HardcodedMiniboxingSimple_JavaRT_3 extends BaseTest {
  ...
  private[this] object TestList {
    ...
    def list_find(l: MBList[Int]): Boolean = {
      ...
      var b = true
      while (i < testSize) {
        b = b ^ l.contains_J(IntToMinibox(i), INT)
        i += 10000
      }
      b
    }
    ...
    def list_find_LONG(l: MBList[Long]): Boolean = {
      ...
      var b = true
      while (i < testSize) {
        b = b ^ l.contains_J(IntToMinibox(i), LONG)
        i += 10000
      }
      b
    }
    ...
    def list_find_SHORT(l: MBList[Short]): Boolean = {
      ...
      var b = true
      while (i < testSize) {
        b = b ^ l.contains_J(IntToMinibox(i), SHORT)
        i += 10000
      }
      b
    }
  ...
  private[this] object TestArray {
    ...
    def array_find(a: MBResizableArray[Int]): Boolean = {
      ...
      var b = true
      while (i < testSize) {
        b = b ^ a.contains_J(IntToMinibox(i), INT) // TODO: Does this cost much?
        i += 10000
      }
      b
    }
    ...
    def array_find_LONG(a: MBResizableArray[Long]): Boolean = {
      ...
      var b = true
      while (i < testSize) {
        b = b ^ a.contains_J(LongToMinibox(i), LONG) // TODO: Does this cost much?
        i += 10000
      }
      b
    }
    ...
    def array_find_SHORT(a: MBResizableArray[Short]): Boolean = {
      ...
      var b = true
      while (i < testSize) {
        b = b ^ a.contains_J(IntToMinibox(i), SHORT) // TODO: Does this cost much?
        i += 10000
      }
      b
    }
  ...
  def testHardcodedMiniboxingSimple_JavaRT_3(megamorphic: Boolean) = {
    import TestArray._
    import TestList._

    val transformation = "miniboxed JavaRT 3 " + (if (megamorphic) "mega" else "mono")
