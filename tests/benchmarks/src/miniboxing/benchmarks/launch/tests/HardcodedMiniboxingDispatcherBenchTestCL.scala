...
trait HardcodedMiniboxingDispatcherBenchTestCL extends BaseTest {
  ...
  private[this] object TestList {
    def list_insert(): DispList[Int] = {
      ...
      var i = 0
      while (i < testSize) {
        l = DispListFactory.newDispList_J[Int](int2minibox(i), l, Dispatchers.IntDispatcher)
        i += 1
      }
      l
    }
    ...
    def list_insert_LONG(): DispList[Long] = {
      ...
      var i = 0
      while (i < testSize) {
        l = DispListFactory.newDispList_J[Long](long2minibox(i), l, Dispatchers.LongDispatcher)
        i += 1
      }
      l
    }
    ...
    def list_insert_DOUBLE(): DispList[Double] = {
      ...
      var i = 0
      while (i < testSize) {
        l = DispListFactory.newDispList_J[Double](int2minibox(i), l, Dispatchers.DoubleDispatcher)
        i += 1
      }
      l
    }
