...
import miniboxing.benchmarks.hardcoded.noinline._

trait HardcodedMiniboxingSimpleNI extends BaseTest {
  ...
  def testHardcodedMiniboxingSimpleNI(megamorphic: Boolean) = {
    import TestArray._
    import TestList._

    val transformation = "miniboxed standard NI " + (if (megamorphic) "mega" else "mono")
